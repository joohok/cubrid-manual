...
**flashback_timeout**
 ...
 flashback 유틸리티에서 트랜잭션 식별자를 입력할 수 있는 제한 시간을 지정한다. 지정된 제한 시간 내에 변경 사항을 되돌릴 트랜잭션을 입력하지 않으면 오류 메시지가 출력되고, flashback 유틸리티는 종료된다. 제한 시간이 없을 경우 내부적으로 flashback 유틸리티에서 필요한 보관 로그 볼륨들이 삭제되지 않는 문제가 발생할 수 있기 때문에 제한 시간을 제거할 수 있는 방법은 제공되지 않는다. 기본값은 300초이다.
